...
- Beatdown name
- Day of week
- Event ID (to handle multiple events at the same location on the same day)

The ID format is: `{region}_{name}_{dayOfWeek}_{eventId}`

Example: `naperville-iron-lion-saturday-32490`

The ID is generated by converting these values to lowercase and replacing spaces/special characters with hyphens.

**Note:** The ID format was updated to include `eventId` to prevent collisions when multiple events occur at the same location on the same day. The script automatically migrates existing documents from the old format to the new format. 